...
$('#test_year').change(function(){
	selectMonthListCall(this.value);
	search(currPage,10);
	list_call = 0;
	currPage = 1;
});
...
function testCategoryCall(upperCate,Cate,id){
	...
	$.ajax({
		...
		success : function(data){
			...
			id.empty();
			$('#test_year').empty();
			selectMonthListCall($('#test_year').val());
			id.append(txt);
		},
		error : function(e){
			console.log(e);
...
function selectYearListCall(test_cate_no){
	console.log(test_cate_no+' 관련한 연도를 불러옴');
	$.ajax({
...
//1.시험카테고리 건들때
$('#test_cate_no').change(function(){
	console.log('시험카테고리 건들였음 '+this.value);
	selectYearListCall(this.value);
